<div class="container">
    ...
    if ($this->session->userdata('_delete')) {
        ?>
        <div class="alert alert-success"><i class="glyphicon glyphicon-ok-sign"></i> Plot Successfully Deleted</div>
        <?php
    } elseif ($this->session->userdata('_notdelete')) {
        ?>
        <div class="alert alert-warning"><i class="glyphicon glyphicon-warning-sign"></i> Plot could not be deleted.plot already sold </div>
        <?php
    }
...
// jQuery expression for case-insensitive filter
    ...
    <h3><i class="glyphicon glyphicon-list"></i> Plot List</h3>
    <?php
    //debugPrint($plotList);
    ?>
    <div id="dynamic">
        <table class="table table-bordered table-striped tablesorter" id="myTable">
            ...
            <tbody>
                ...
                <?php
                $i = 0;
                foreach ($plotList as $list) {
                    $i++;
                    ?>
                    <tr class="dlist">
                        <td class="checkArea"><input type="checkbox" name="checkId[]" value="<?php echo encode($list[dbConfig::TABLE_PLOT_ATT_PLOT_ID]); ?>"/></td>
                        <td>
                            <?php
                        ...
                        <td><?php echo $list[dbConfig::TABLE_PLOT_ATT_PLOT_NAME] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_PLOT_ATT_PLOT_ADDESS] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_PLOT_BLOCK_ATT_BLOCK_NAME] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_PLOT_FACING_ATT_FACE_NAME] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_PLOT_CONDITION_ATT_CONDITION_NAME] ?></td>
                        <td>
                            <?php
                            if ($list[dbConfig::TABLE_PLOT_STATUS_ATT_STATUS_NAME] == 'Sold') {
                                ?>
                                <span class="alert alert-danger" style="padding: 5px;"><?php echo $list[dbConfig::TABLE_PLOT_STATUS_ATT_STATUS_NAME] ?></span>
                                <?php
                            } else {
                                ?>
                                <span class="alert alert-success" style="padding: 5px;"><?php echo $list[dbConfig::TABLE_PLOT_STATUS_ATT_STATUS_NAME] ?></span>
                                <?php
                            }
                            ?>
                        </td>
                        <td><?php echo $list[dbConfig::TABLE_PLOT_ATT_PLOT_ASKING_MIN_PRICE] ?>-<?php echo $list[dbConfig::TABLE_PLOT_ATT_PLOT_ASKING_PRICE] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_CURRENCY_ATT_CURRENCY_CODE] ?></td>                        
                        <td><a href="<?php echo base_url() . siteConfig::CONTROLLER_PLOT . siteConfig::METHOD_PLOT_VIEW_PLOT . encode($list[dbConfig::TABLE_PLOT_ATT_PLOT_ID]); ?>" class="btn btn-info">View</a></td>
                    </tr>
                    <?php
                }
                ?>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" onclick="sendMailPlot()">Send</button>
                    </div>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
        </div>
