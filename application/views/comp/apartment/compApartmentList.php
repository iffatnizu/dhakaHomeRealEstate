<div class="container">
    ...
    if ($this->session->userdata('_delete')) {
        ?>
        <div class="alert alert-success"><i class="glyphicon glyphicon-ok-sign"></i> Apartment Successfully Deleted</div>
        <?php
    }
    elseif ($this->session->userdata('_notdelete')) {
        ?>
        <div class="alert alert-warning"><i class="glyphicon glyphicon-warning-sign"></i> Apartment could not be deleted.Apartment already sold </div>
        <?php
    }
...
// jQuery expression for case-insensitive filter
    ...
    <h3><i class="glyphicon glyphicon-list"></i> Apartment List</h3>
    <?php
    //debugPrint($apartmentList);
    ?>
    <div id="dynamic">
        <table class="table table-bordered table-striped tablesorter" id="myTable">
            ...
            <tbody>
                ...
                <?php
                $i = 0;
                foreach ($apartmentList as $list) {
                    $i++;
                    ?>
                    <tr class="dlist">
                        <td class="checkArea"><input type="checkbox" name="checkId[]" value="<?php echo encode($list[dbConfig::TABLE_APARTMENT_ATT_APARTMENT_ID]); ?>"/></td>
                        <td>
                        ...
                        <td><?php echo $list[dbConfig::TABLE_APARTMENT_ATT_APARTMENT_NAME] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_APARTMENT_ATT_APARTMENT_ADDRESS] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_APARTMENT_SIZE_ATT_SIZE_VALUE] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_APARTMENT_CONDITION_ATT_CONDITION_NAME] ?></td>
                        <td>
                           
                            <?php
                            if ($list[dbConfig::TABLE_COMMERCIAL_STATUS_ATT_STATUS_NAME] == 'Sold') {
                                ?>
                                <span class="alert alert-danger" style="padding: 5px;"><?php echo $list[dbConfig::TABLE_APARTMENT_STATUS_ATT_STATUS_NAME] ?></span>
                                <?php
                            } else {
                                ?>
                                <span class="alert alert-success" style="padding: 5px;"><?php echo $list[dbConfig::TABLE_APARTMENT_STATUS_ATT_STATUS_NAME] ?></span>
                                <?php
                            }
                            ?>
                        </td>
                        <td><?php echo $list[dbConfig::TABLE_APARTMENT_ATT_APARTMENT_ASKING_MIN_PRICE] ?>-<?php echo $list[dbConfig::TABLE_APARTMENT_ATT_APARTMENT_ASKING_PRICE] ?></td>
                        <td><?php echo $list[dbConfig::TABLE_CURRENCY_ATT_CURRENCY_CODE] ?></td>                        
                        <td><a href="<?php echo base_url() . siteConfig::CONTROLLER_APARTMENT . siteConfig::METHOD_APARTMENT_VIEW_APARTMENT . encode($list[dbConfig::TABLE_APARTMENT_ATT_APARTMENT_ID]); ?>" class="btn btn-info">View</a></td>
                    </tr>
                    <?php
                }
                ?>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-primary" onclick="sendMailApartment()">Send</button>
                    </div>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
        </div>
